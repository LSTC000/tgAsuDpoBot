...
@dp.message_handler(content_types=types.ContentTypes.TEXT, state=UpdateQuestionAndAnswerStatesGroup.enter_new_answer)
async def enter_new_answer(message: types.Message, state: FSMContext) -> None:
    user_id = message.from_user.id
    text = message.text

    # Clear last inline keyboard.
    await clear_last_ikb(user_id=user_id, state=state)
    # Check new answer on valid.
